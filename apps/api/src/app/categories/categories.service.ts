...
import {
  ...
  UpdateCategoryDto,
  CreateBatchCategoriesResultDto,
  ListCategoriesResultDto,
  SyncCategoryChildrenResultDto,
} from '@org/contracts';
import {
  CategoryNode,
  CategoryRow,
...
export class CategoriesService {
  ...
  async createCategory(input: CreateCategoryDto) {
    ...
    if (input.parentId) {
      await this.getExistingCategory(input.parentId);
    }
  ...
  async createBatchCategories(input: {
    categories: CreateBatchCategoryDto[];
  }): Promise<CreateBatchCategoriesResultDto> {
  ...
  ): Promise<ListCategoriesResultDto> {
    const pageSize = input.pageSize ?? DEFAULT_PAGE_SIZE;

    if (input.paginationType === 'cursor') {
      return this.listCategoriesWithCursor(input, pageSize);
    }
  ...
  async updateCategory(id: string, input: UpdateCategoryDto) {
    ...
    const merged = {
      ...
      description:
        input.description !== undefined
          ...
          : current.description,
      imageUrl:
        input.imageUrl !== undefined ? input.imageUrl || null : current.imageUrl,
      metaTitle:
    ...
    if (input.parentId !== undefined) {
      await this.validateHierarchyChange(id, input.parentId || null);
    }
  ...
  async deleteCategory(id: string) {
    const current = await this.getExistingCategory(id);
    const hasChildren = await this.category.findFirst({
  ...
  async syncCategoryChildren(
    input: SyncCategoryChildrenDto,
  ): Promise<SyncCategoryChildrenResultDto> {
  ...
  private queryCategories(
    ...
    return this.category.findMany({
      where: this.categoryWhere(input, cursorName, cursorId, isBackward),
      include: CATEGORY_RESPONSE_INCLUDE,
      orderBy: [
  ...
  private async syncCreateAll(
    ...
    const results = await Promise.all(
      items.map(async (item) => {
        try {
          const result = await this.createCategory({ ...item, parentId });
          if (!result) {
  ...
  private async syncUpdateAll(
    ...
    const results = await Promise.all(
      items.map(async (item) => {
        try {
          await this.updateCategory(item.id, item.changes);
          return { id: item.id };
        } catch (error) {
          return {
            id: item.id,
  ...
  ): SyncCategoryChildrenResultDto['status'] {
    const failures =
  ...
  private validateBackendCategory(input: {
    name: string;
  ...
  private async getDescendants(parentId: string): Promise<CategoryRow[]> {
    ...
    while (frontier.length > 0) {
      const children = await this.category.findMany({
  ...
  private get category(): CategoryDelegate {
    return (this.prisma.client as unknown as { category: CategoryDelegate })
      .category;
  }
}
